  ...
  if ($user->hasAttribute('popup', 'sf_admin/sf_asset/navigation'))
  {
    switch ($user->getAttribute('popup', null, 'sf_admin/sf_asset/navigation'))
    {
      ...
      case 3:
        ...
        return link_to_function($text, "window.opener.CKEDITOR.tools.callFunction('".$user->getAttribute('CKEditorFuncNum', 1, 'sf_admin/sf_asset/navigation')."', '" . $asset->getUrl() . "'); window.close()");
    }
  }
  else
  {
...
function get_asset_url($type = null)
{
  ...
  return url_for('@sf_asset_library_list?popup=3'.$add);
}
...
function input_sf_asset_image_tag($name, $value = null, $options = array())
{
  ...
  return '<a class="sf_asset_inputs_image" href="#" rel="{url: \'' . $url . '\', name: \'' . $name . '\', type: \'' . $options['type'] . '\'}">' .
